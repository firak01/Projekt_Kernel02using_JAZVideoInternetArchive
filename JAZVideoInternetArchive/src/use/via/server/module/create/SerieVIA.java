...
public class SerieVIA  extends DocumentCreatorZZZ{
		
	public SerieVIA(){
		//Wird benötigt, um einfach so per ReflectionAPI ein Dokument zu erzeugen.
	}
	public SerieVIA(KernelNotesZZZ objKernelNotes){
		super(objKernelNotes);
	}
	public SerieVIA(KernelNotesZZZ objKernelNotes, MapperStoreServerVIA objStoreHTTPMapper){
		super(objKernelNotes);
	...
	public boolean validateMapperStore() throws ExceptionZZZ{
		...
		main:{
			//try{
				...
				DataStoreZZZ objData = objMapper.getDataStoreSerie();	
...
//		### CarrierID als Schlüssel des Parentdokuments. Merke 1 Movie kann sich auf n Carriern befinden
	...
	public Document searchDocumentExisting() throws ExceptionZZZ {
		...
		main:{
			try{
				...
				String sTitle = objData.getValueString("Title", 0);
				if(StringZZZ.isEmpty(sTitle)){
					ExceptionZZZ ez = new ExceptionZZZ("No 'title' of the serie was passed by http-request-parameter.", iERROR_RUNTIME, this, ReflectCodeZZZ.getMethodCurrentName());
					throw ez;
				}
				
							
				//++++++++ die Suche
				//20090321 Die Suche muss unabhängig von der Groß-/Kleinschreibung der Serie sein
				String sTitleField = objData.getMetadata("Title", DataFieldZZZ.FIELDNAME);				
				String sSearch = "@Lowercase(Form)=\""+ sForm.toLowerCase() + "\" & @trim(@Lowercase(" + sTitleField + "))=\"" + sTitle.toLowerCase() + "\"";
				...
				if(col.getCount() >= 1){
					NotesReportLogZZZ.write(NotesReportLogZZZ.DEBUG, "A serie document was found");		
					docReturn = col.getFirstDocument();						
					break main;
				}else{
					NotesReportLogZZZ.write(NotesReportLogZZZ.DEBUG, "NO serie document was found");								
				}				
			}catch(NotesException ne){
				ExceptionZZZ ez = new ExceptionZZZ(ne.text, iERROR_RUNTIME, this, ReflectCodeZZZ.getMethodCurrentName());
				throw ez;
	...
	public static Vector getViewnameEmbeddedAllUsed() {
		Vector vecReturn = new Vector();
		vecReturn.add("viwMovieBySerieSingleCategoryVIA");
		return vecReturn;
	}
}//END class