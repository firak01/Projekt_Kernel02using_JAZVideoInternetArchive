...
public class JTextFieldListening4ComponentSelectionResetVIA extends KernelJTextFieldListening4ComponentSelectionResetZZZ  implements IListenerFileSelectedVIA{
	...
	public JTextFieldListening4ComponentSelectionResetVIA(KernelZZZ objKernel, KernelJPanelCascadedZZZ panelCascaded, String sTextInitial) {
	...
	//			dadurch soll verhindert werden, dass gerade eingegebener Text gelöscht wird, auch wenn z.B. noch keine Datei markiert worden ist.
	...
	//			FGL 20080816 Lösungsversuch "neue CD"  über eine Hilfmethode ... .isDriveChanged();
	...
	//			boolean btemp = CommonUtilVIA.isDriveChanged(this.getPanelParent(), sRootNew, this.sRootPrevious);
				...
				this.sRootPrevious = sRootNew;  //damit es beim nächsten mal gefüllt ist
				ReportLogZZZ.write(ReportLogZZZ.INFO, "Root previous = " + this.sRootPrevious);
				
				//if(btemp==false) break main; //es gibt also kein neues Laufwerk/keinen neuen Datenträger, ergo nix aktualisieren.
			
				
							
				//Wenn das nicht überschreiben wird, so bekommt man vom JCombo das Verzeichnis geliefert
