   ...
   public String generateContent4Success(Document docCarrier, Document docFile, Document docSerie, Document docMovie, String sContentTypeIn) throws ExceptionZZZ{
	   ...
	   main:{
		   ...
		   //###################################################################################
			...
			//############
			//2. Objekt, das dann sp�ter an einen ContentWriter �bergeben werden kann
			...
			//Besser ist es, das CarrierDokument w�rde am Ziel (dem JavaClient)  wieder in eine Art dummyNotesDocumentXml basiert entpackt und man 
...
//					+++++++++ ZIEL: Ausgabe als Html und die Platzhalter f�r die Muster-Datei f�llen.					
...
private String generateContent4SuccessHtml(IKernelZZZ objKernel, ContentFileZZZ objContentStore) throws ExceptionZZZ {
	...
	/*++++ nur f�r eine Zwischenausgabe. NICHT L�SCHEN
	...
	System.out.println("Verwendetes Encoding Format: " + format.getEncoding());
	xmlout.setFormat(format);  //Das muss man wieder zur�ckgeben, sonst funktioniert es nicht
	try {
	...
	NotesReportLogZZZ.write(NotesReportLogZZZ.DEBUG, "Objekt KernelWriterHtmlByFileZZZ erfolgreich erstellt und mit dem ContentStore-Objekt gef�llt.");
	
	//5. als String zur�ckgeben
	//objWriterHTML.toFile("c:\\fglkernel\\kerneltest\\test.html");  //Create File (NUR F�R TESTZEWECKE)
	sReturn = objWriterHTML.toStringContent();  //Merke: .toString() ist f�r gaaaanz andere Dinge reserviert !!!
	return sReturn;
}
...
private String generateContent4SuccessXml(Document docCarrier, Document docFile, Document docSerie, Document docMovie, String sReturn, IKernelZZZ objKernel, ContentXmlZZZ objContentXmlStore) throws ExceptionZZZ {
	...
	//				s.o. PipedReader pipeIn =null;
					...
					/*  Merke: Die Methode notesdocument.generateXML() bringt im Servlet immer den Server zum Absturz
						//TODO: Daf�r eine eigene Klasse in einem eigenen Projekt entwickeln, das Notes und JDOM .jar Files beinhaltet. Und auf das JDOM-Dokument wie auf Notesdocumente zugreift.
						//a) CarrierDokument					
	...
	//					s.o. pipeIn.connect(pipeOut);
						...
						NotesReportLogZZZ.write(NotesReportLogZZZ.DEBUG, "FileWriter erzeugt. erzeuge nun XML (Carrier-Dokument) f�r den PipeWriter-Stream.");
						...
						*/
						...
						NotesReportLogZZZ.write(NotesReportLogZZZ.DEBUG, "JDom als Variable 'documentmovie' an den Store ContentXmlZZZ �bergeben.");
						...
						NotesReportLogZZZ.write(NotesReportLogZZZ.DEBUG, "JDom als Variable 'documentfile' an den Store ContentXmlZZZ �bergeben.");
						...
						if(docSerie!=null){
							...
							NotesReportLogZZZ.write(NotesReportLogZZZ.DEBUG, "JDom als Variable 'documenserie' an den Store ContentXmlZZZ �bergeben.");
						}
