 ...
 * Die notwendigen Informationen werden �ber das DocumentData-Objekt zur Verf�gung gestellt.
 * 
 * Merke:
 * Es wird nicht nur ein Dokument hiermit erstellbar, sondern alle Dokumente, f�r die es auch ein entsprechendes DataStore-Objekt gibt.
 * 
...
public abstract class MapperStoreHttpZZZ extends KernelUseObjectZZZ{
	...
	//Hinweis: Diese DataStore-Objekte enthalten nur die Daten, die von A nach B �bertragen werden. Dazu kommen noch Daten, die an alle Dokumente �bertragen werden sollen und aus "Umgebungsvariablen" oder "statischen Werten" bestehen.
	protected HashMap hmDataStore = new HashMap();
	protected DataStoreZZZ objDataMeta = new DataStoreZZZ("$ALL"); //DataStore f�r die Dokument Metadaten, die in alle Dokumente kommen, z.B. Ersteller
	...
	/** !!! Diese MEthode f�hrt alle zu �berschreibenden MEthoden aus. Sie muss nach der erzeugung des Objekts ausgef�rht werden.
	 *        Dieses Vorgehen erlaubt es den Klassen, die von dieser Klasse erben, ihre speziellen F�higkeiten (z.B.Protokollierung mit  NotesLogZZZ) zu nutzen, die es f�r diese Klasse noch nicht gibt.
	* @throws ExceptionZZZ
	* 
	* lindhauer; 30.01.2008 13:13:04
	...
	public void loadAll() throws ExceptionZZZ{
		...
		//Die DataFieldStruktur f�r alle DataStore Objekte laden
	...
	/** Gibt alle Aliasnamen f�r das HTTP-Mapping zur�ck, bezogen auf einen bestimmten Aliasnamen eines DataStores.
	 *  Merke: Das Mapping wird in .loadFieldMapAll() durchgef�hrt.
	...
	* lindhaueradmin; 28.02.2007 10:52:55
	...
	public ArrayList getAliasMappedAll(String sAliasDataStore) throws ExceptionZZZ{
		ArrayList listaReturn = null;
			//TODO; Falls sAliasDataStore leer ist, sollen alle Werte zur�ckgegeben werden.
	...
	* lindhaueradmin; 27.11.2006 08:26:16
		...
		HashMap objReturn = new HashMap();
			...
			while(enumeration.hasMoreElements()){
				...
				if(! StringZZZ.isEmpty(sParam)){
					sValue = req.getParameter(sParam); //TODO: Hier die Verarbeitung von mehrerern gleichnamigen Parametern mit .getParameters(...) einf�gen.
					                                                   //           Dazu muss aber auch die Speicherung gel�st werden. Vielleicht JSON als L�sung wie in der Hashmap noch weitere Mehrfachwerte gespeichert werden sollen...
					                                                   //           Hier eine weitere HashMap f�r Mehrfachwerte einzuf�hren, wird zu extrem komplexen Datenstrukturen f�hren, FGL 20070609
	...
	public String getParameterFieldByAlias(DataStoreZZZ objStore, String sAlias, String sMetadataType) throws ExceptionZZZ{
		String sReturn = null;
			...
			//!!! Der Alias ist nun der Alias f�r den HTTP-Parameter, bzw. die SwingKomponente (als Quelle) oder das Notesdokument (als Ziel)
	...
	public boolean isAliasFieldStructureAvailable(String sStoreNameIn, String sAliasSearchedFor) throws ExceptionZZZ{
		boolean bReturn = false;
			...
			//Das ist aber nicht der Aliasname, der in der Field-Struktur gespeichert ist, sondern ein �bergeordneter
