...
public class MapperStoreClientVIA extends MapperStoreHttpZZZ {
	...
	public MapperStoreClientVIA(IKernelZZZ objKernel) throws ExceptionZZZ {
		super(objKernel);
		this.loadAll(); //Ausführen der überschriebenen abstrakten Methoden.
	}
	...
	public void loadDataStoreAll() throws ExceptionZZZ {
		//TODO Idee: Das Panel �bergeben im Konstruktor //DataStoreZZZ objDataExportPanel = new DataStoreZZZ("ExportPanel");  //DataStore f�r das "Carrier - Dokument"
		DataStoreZZZ objDataExportPanel = new DataStoreZZZ("ExportPanel");  //DataStore f�r das "Carrier - Dokument"
	...
	public HashMap loadFieldMapAll() throws ExceptionZZZ {
		...
		main:{
			/* Aufgabe:
			 * Eine Datenstruktur zur Vef�gung stellen, in der die Parameter gemapped werden: liste(sMaske) = sParameter_namen.
			   Aufgrund des HTTP-Parameternamens soll Alias-Parametername ermittelt werden (damit das DataStore-Objekt dazu sp�ter den passenden Feldnamen hat.
			   
			   Diese Datenstruktur wird dann in eine HashMap gepackt, welche als Key den Namen des Datastores besitzt.
			   
			/* Merke1: Aus Dokumentationsgr�nden (und vielleicht ist das ja auch sp�ter so verwendbar, dass der Feldname beim Laden der DataStructure nicht mehr "hart ausprogrammiert" zu werden braucht)
			 *  wird der Feldname als 3. Parameter einfliessen.
			
			/* Merke2: Weil es keine einfache (!nicht wie f�r JTabel die Tabellen Modelle), schon fertige Tabellenstruktur gibt (hab danach mehrere Stunden gesucht),
			*  entscheide ich mich daf�r, dass die drei Werte: Feldname-Alias, HPPT-Name, Notes-Feldname in eine dreidimensionale ArrayList gepackt wird.				
			*  
			*    TODO: Klasse, die Methoden bietet, auf die mehrdimensionale ArrayList zuzugreifen.
			...
			alsFielda[MapperStoreHttpZZZ.iPARAMETER_FIELDNAME].add("CarrierID"); //Swing-Komponenten-alias. D.h. Die Beizeichnung eines DataFieldZZZ, ggf. ist darin eine Formel eingebaut.		
																														  //Merke: Hier in diesem speziellen Fall ist das sogar eine Formel, die ausgef�hrt werden muss (mit JEXL) 
	...
	public void loadFieldDataStoreFieldStructureAll() throws ExceptionZZZ {
		// TODO Idee: Das bef�llen der Feldstruktur ist eigentlich Aufgabe der Klasse DataStoreZZZ
//		Merke: Die Feldnamen korrespondieren mit den Feldnamen in einer Notes Maske
		...
		//TODO: Die Feldnamen f�r die NotesFelder werden schon im Servlet-gemapped. Dieses Mapping hier �bergeben und dann die Werte dynamisch auslesen pro Alias und Datastore.
		
		//### ExportPanel (Merke: Das ist f�r eine Swing-Maske)
		DataFieldZZZ objField = new DataFieldZZZ("CarrierID");  //MErke: Diese Aliaswerte entsprechen den gemappten Werten aus loadFileMapAll() f�r MapperStoreHttpZZZ.iPARAMETER_FIELDNAME
		objField.Datatype=DataFieldZZZ.sSTRING;                //Datentyp. Wird beachtet beim Setzen der Werte ins Ziel. 
																				//               Entspricht dem R�ckgabewert der Datamethod beim Auslesen der Werte aus der Quelle.
		...
		//a) @Z-Methode: Mit JEXL Unterst�tzung diese Formel aufl�sen
		...
		//Ziel der Formel ist, dass der Wert nur gesetzt wird, wenn es einen Wert f�r CarrierSequenze gibt.
		//objField.Fieldmethod = "CarrierSequenze<?/>CarrierCreated<+/>'#'<+/>CarrierSequenze<:/>''";  //Dat hab ich mir selber ausgedacht. Heisst: WEnn CarrierSequenz gef�llt ist, dann ... ansonsten CarrierID''.
		//TODO: DAS REALISIEREN 
		//objField.Fieldmethod = "CarrierID<?/>CarrierID<:/>CarrierCreated<+/>'#'<+/>CarrierSequenze";  //Dat hab ich mir selber ausgedacht. Heisst: WEnn CarrierID gef�llt ist, dann ... ansonsten rechne aus''.
		objField.Fieldname="textCarrierId";        ////Im Notes: DerFeldname der Maske, Im Swing: Der Aliasname der Komponente, wie er im ZKernel hinzugef�gt wurde.
		objField.Zclass="@Z"; 									 //!!!Hier: Es soll eine @Z - Methode ausgef�hrt werden, sonst: Klasse der Komponente
		//objField.Zmethod="CarrierID<?/>CarrierID<:/>CarrierCreated<+/>'#'<+/>CarrierSequenze";  //Dat hab ich mir selber ausgedacht. Heisst: WEnn CarrierID gef�llt ist, dann ... ansonsten rechne aus''.
		objField.Zmethod="CarrierID<?/>CarrierID<:/>''";  //Dat hab ich mir selber ausgedacht. Heisst: WEnn CarrierID gef�llt ist, dann ... ansonsten rechne aus''.
		...
		objField.Fieldname="labelCarrierTitle";                            //Im Notes: DerFeldname der Maske, Im Swing: Der Aliasname der Komponente, wie er im ZKernel hinzugef�gt wurde.					
		this.getDataStoreExportPanel().setField(objField);
